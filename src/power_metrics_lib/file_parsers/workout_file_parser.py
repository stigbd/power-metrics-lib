...
def parse_workout_file(file_path: str) -> Workout:
    ...
    for block in blocks_in_workout:
        if block.tag == "SteadyState":
            ...
            if block.tag == "Cooldown":
                blocks.append(
                    Cooldown(
                        start_power=power_low, end_power=power_high, duration=duration
                    )
                )
                ...
                # "Ramp"
                blocks.append(
                    Ramp(start_power=power_low, end_power=power_high, duration=duration)
                )
            ...
            blocks.append(
                Interval(
                    repeat=repeat,
                    work_power=on_power,
                    work_duration=on_duration,
                    rest_power=off_power,
                    rest_duration=off_duration,
                )
            )
